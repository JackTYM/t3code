...
export function renderBootServiceShim(plan: BootServicePlan): string {
  ...
  return [
    ...
    // 0 = hidden window, False = do not block; the task completes immediately
    // while the server keeps running, which is why the task is registered with
    // an unlimited execution time limit rather than a running-instance policy.
    `shell.Run ${quoteVbsString(`cmd /c "${command} ${redirect}"`)}, 0, False`,
    "",
  ].join("\n");
}
